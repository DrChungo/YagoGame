...
public static class PhaseHelpers
{
    public static List<EnemyDef> GenerateRoom(
        ...
        Random rng)
    {
        var parsed = tierWeights
            .Select(kv => (tier: int.Parse(kv.Key), weight: kv.Value))
            .ToList();
        ...
        return Enumerable.Range(0, 3)
            .Select(_ =>
            {
                ...
                return enemies.Where(e => e.Tier == roll)
                              .OrderBy(_ => rng.Next())
                              .First();
            })
            .ToList();
    ...
    public static List<CombatService.EnemyInstance> ToInstances(
        List<EnemyDef> enemies)
    {
        return enemies.Select(e =>
            new CombatService.EnemyInstance(
                e.Name,
                Clone(e.Stats),
                e.Attack))
            .ToList();
